...
{
    private const string MethodTestCasePattern = "void Method\\(\\)\\s*{\\r\\n(?<Source>.*)}";
    private const string SimpleTestCasePattern = "// <auto-generated/>\\s*#nullable enable\\s*namespace Test;\\s*(static )?partial class Class\\s*{\\r\\n(?<Source>.*)}";
    private const string DefaultTestPathPattern = "Test\\.Class\\.(MethodAsync\\.)?";
    ...
    private static Target SourceToTarget(GeneratedSourceResult source)
    {
        ...
        var sourceToStore = sourceText;
        if (match.Success)
        {
            sourceToStore = TestHelper.ChangeIndentation(match.Groups["Source"].Value, TestHelper.RemoveIndentation);
...
#endif
            match = methodMatch.Match(sourceToStore);
    ...
    [GeneratedRegex(MethodTestCasePattern, RegexOptions.Singleline | RegexOptions.Compiled)]
    private static partial Regex DefaultMethodTestRegex();
